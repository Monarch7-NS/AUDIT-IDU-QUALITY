...
def generate_audit_output():
    ...
    score_completude = max(
        0,
        (total_modules_maquette - missing_modules) / total_modules_maquette * 100
    ) if total_modules_maquette else 0
    ...
    score_conformite = max(
        0, 100 - (anomalies_sequence / total_modules_maquette * 10)
    )  # Heuristic penalty
    ...
    for _, row in df_unicite.iterrows():
        desc = (
            f"Chevauchement pour {row['enseignant']}"
            f" entre {row['debut_A']} et {row['debut_B']}"
        )
        anomalies.append({
            "module": row['module_A'],
            "type": "Unicité",
    ...
    for _, row in df_exactitude[ecarts > 20].iterrows():
        if row['heures_maquette'] == 0:
            desc = (
                f"Séances {row['type_seance']} planifiées sur ADE"
                f" ({row['heures_ade']}h) alors que la maquette prévoit 0h."
            )
        else:
            desc = (
                f"Écart horaire de {round(row['ecart_pourcentage'])}%"
                f" sur le volume {row['type_seance']}."
            )
        anomalies.append({
    ...
    for _, row in df_coherence.iterrows():
        desc = (
            f"L'enseignant ADE ({row['enseignant_ade']})"
            f" n'est pas le responsable officiel ({row['responsable_officiel']})."
        )
        anomalies.append({
            "module": row['module_code'],
            "type": "Cohérence",
    ...
    print("Export JSON généré avec succès dans data/audit_output.json")
