...
def _check_teacher_overlaps(events: list[dict]) -> list[OverlapAnomaly]:
    ...
    # Group/promo prefixes that are NOT real teachers
    group_prefixes = (
    ...
    for evt in events:
        for teacher in evt.get("teachers", []):
            ...
            if not name:
                continue
            # Skip group identifiers mistakenly parsed as teachers
            if any(name.startswith(prefix.upper()) for prefix in group_prefixes):
                continue
