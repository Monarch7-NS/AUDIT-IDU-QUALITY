...
def audit_completude(df_ade, df_maquette):
    ...
    taux = (len(maquette_modules) - len(missing_in_ade)) / len(maquette_modules) * 100 if len(maquette_modules) > 0 else 0
...
def audit_exactitude(df_ade, df_maquette):
    ...
    df_maq = df_maq.melt(id_vars=['code_module'], var_name='type_seance', value_name='heures_maquette')
    ...
    # Merge
    df_merged = pd.merge(df_maq, df_ade_agg, on=['code_module', 'type_seance'], how='outer').fillna({'heures_maquette': 0, 'heures_ade': 0})
...
if __name__ == "__main__":
    ...
    print(f"\n[Cohérence] {len(df_coherence)} séances où l'enseignant ADE n'est pas le responsable officiel.")
