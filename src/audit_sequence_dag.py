...
def build_dags_and_nodes(df_seq):
    graphs = {}
    nodes_in_rules = {}
    df_seq = df_seq.dropna(subset=['module_precedent', 'type_precedent', 'numero_precedent', 'module_suivant', 'type_suivant', 'numero_suivant'])
...
def audit_theoretical_dags(graphs):
    ...
    for module, G in graphs.items():
        ...
        if not is_dag:
            ...
            anomalies.append({'module_code': module, 'type_anomalie': 'Structure', 'description': conclusion, 'criticite': 'BLOQUANTE'})
        elif n_components > 1:
            status = "Cassé"
            conclusion = f"Graphe morcelé en {n_components} composantes déconnectées (chaîne brisée)."
            anomalies.append({'module_code': module, 'type_anomalie': 'Structure', 'description': conclusion, 'criticite': 'BLOQUANTE'})
...
def validate_and_find_orphans(graphs, nodes_in_rules, module_status, df_ade):
    ...
    df['numero_infere'] = df.groupby(['module_code', 'type_seance'])['debut'].rank(method='dense').astype(int)
    ...
    for module, G in graphs.items():
        ...
        for o in orphelins_mod:
            ...
            anomalies.append({
                ...
                'description': f'Session {o} planifiée dans ADE mais absente du graphe de dépendance.',
                'criticite': 'MOYENNE'
            })
...
if __name__ == "__main__":
    ...
    ade_anomalies, orphelines, conformite_stats = validate_and_find_orphans(graphs, nodes_in_rules, module_status, df_ade)
    ...
    incomplet_pct = (status_counts['Incomplète / Invalide'] / total_modules * 100) if total_modules > 0 else 0
    
    taux_moyens_violation = sum([stats['taux_violation'] for stats in conformite_stats.values()]) / len(conformite_stats) if conformite_stats else 0
    
    global_audit_stats = {
        'total_modules_audites': total_modules,
        'modules_sains': {'count': status_counts['Sain'], 'pourcentage': round(sain_pct, 2)},
        'modules_casses': {'count': status_counts['Cassé'], 'pourcentage': round(casse_pct, 2)},
        'modules_incomplets': {'count': status_counts['Incomplète / Invalide'], 'pourcentage': round(incomplet_pct, 2)},
        'taux_moyen_violation_conformite': round(taux_moyens_violation, 2),
