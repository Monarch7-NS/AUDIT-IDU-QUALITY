...
def extract_type_seance(title, desc):
    ...
    if re.search(r'\b(CM)\b', title, re.IGNORECASE) or re.search(r'\(CM\)', desc, re.IGNORECASE):
        return 'CM'
    if re.search(r'\b(TD)\b', title, re.IGNORECASE) or re.search(r'\(TD\)', desc, re.IGNORECASE) or re.search(r'_TD', title, re.IGNORECASE):
        return 'TD'
    if re.search(r'\b(TP)\b', title, re.IGNORECASE) or re.search(r'\(TP\)', desc, re.IGNORECASE) or re.search(r'_TP', title, re.IGNORECASE):
        return 'TP'
...
def extract_enseignants(desc):
    ...
    for line in lines:
        ...
        if re.match(r'^[A-ZÉÈÀÇ\s\-]+$', line) and len(line) > 3 and not re.search(r'(IDU|MECA|SNI|EXPORT)', line):
...
def load_and_clean_ade_data(ade_files, maquette_path):
    ...
    df_clean['module_code'] = df_clean.apply(lambda x: extract_module_code(x['Title'], x['Description'], lookup), axis=1)
    df_clean['type_seance'] = df_clean.apply(lambda x: extract_type_seance(x['Title'], x['Description']), axis=1)
